...
class Responder {
  ...
  _throwErrorIfNoKey() {
    if (!(this.postingKey || this.activeKey)) {
      throw (
        new Error('You need to introduce a postingKey to HiveBot\'s constructor')
      );
    }
  }
  ...
  async commentBurn(
    ...
    try {
      permlink = createCommentPermlink(targetUsername, targetPermlink);
      postingWif = this.postingKey || this.activeKey;
      jsonMetadata = JSON.stringify({
        app: `HIVE-Matrix/${HiveBotVersion}`,
        tags: targetTags,
      });
    } catch (error) {
      console.error(error);
    ...
    const operations = [
      ['comment',
        {
          parent_author: targetUsername,
          parent_permlink: targetPermlink,
          author: this.responderUsername,
          permlink,
          title: '',
          body: message + "\n\n<sub>*AI generated content*\nCommands: !hive stop | !hive start | !hive price</sub>",
  ...
  async comment(
    ...
    return this.commentBurn(message,
      targetTags = "",
      targetUsername = this.targetUsername,
      targetPermlink = this.targetPermlink)
    ...
    // return 
    await hivejs.broadcast.commentAsync(
      wif,
      targetUsername,
      targetPermlink,
    ...
    const maxAcceptedPayout = '0.000 HBD';
    const percentHiveDollars = 0;
    const allowVotes = false;
    const allowCurationRewards = false;
    const extensions = [];

    return await hivejs.broadcast.commentOptions(
      wif,
      this.responderUsername,
      permlink,
      maxAcceptedPayout,
